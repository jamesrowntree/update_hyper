...
cp .env.example .env   # then edit .env with your real Tableau Cloud details -- see Appendix H
```

> **The publish step will not run until `.env` is filled in with real values.** `split_by_year.py`, `union_hyper_files.py`, and `generate_metadata.py` need no configuration at all and will run right after `pip install`. Only the last step, `publish_to_tableau_cloud.py`, reads `.env` — it checks for all five required variables up front and refuses to make any network call if any are missing. See Appendix H for what each variable means and how to find your real values.
...
**Requires a filled-in `.env` first** (see Appendix H) — the script checks for it and refuses to run otherwise. Only `Finished_Merged.hyper` gets published — the original `Start.hyper` and the six per-year files are intermediate/demo artifacts and are never uploaded anywhere.

---

## Appendix

*Sections 1–4 above are everything you need to run the pipeline. Everything below is supporting detail: the source dataset, how the scripts work internally, why the approach is fast, whether it still holds up on the current API, verification results, metadata generation, and Tableau Cloud publishing specifics (including what was actually confirmed by publishing to a real site).*
...
### Appendix D: Why this is the fastest way to combine Hyper files
...
### Appendix E: Does this still hold up in 2026? (the post is from Dec 2021)
...
| End-to-end run of both scripts + lossless round-trip verification | Passed (Appendix F) |
...
### Appendix F: Verification performed
...
### Appendix G: Generating metadata for `Finished_Merged.hyper`
...
### Appendix H: Publishing to Tableau Cloud
...
Generated for the update_hyper project. Split/merge scripts tested against `tableauhyperapi==0.0.26359`; publish script tested against `tableauserverclient==0.41` on Python 3.13, including a real publish to a live Tableau Cloud site (see Appendix H).
